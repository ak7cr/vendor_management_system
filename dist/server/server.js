...
app.post('/api/departments', async (req, res, next) => {
    try {
        const dept = req.body;
        await pool.execute('INSERT INTO Department (department_name, respective_manager, no_of_ongoing_projects, no_of_finished_projects, no_of_people_in_department) VALUES (?, ?, ?, ?, ?)', [
...
app.put('/api/departments/:id', async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        const dept = req.body;
        await pool.execute('UPDATE Department SET department_name = ?, respective_manager = ?, no_of_ongoing_projects = ?, no_of_finished_projects = ?, no_of_people_in_department = ? WHERE id = ?', [
            dept.department_name,
            dept.respective_manager,
            dept.no_of_ongoing_projects || 0,
            dept.no_of_finished_projects || 0,
...
app.post('/api/employees', async (req, res, next) => {
    try {
        const emp = req.body;
        await pool.execute('INSERT INTO Employee (name, email, phone, dob, joining_date, department, manager_id, rating_overall) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
            emp.name,
            emp.email,
            emp.phone || null,
            emp.dob || null,
...
app.put('/api/employees/:id', async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        const emp = req.body;
        await pool.execute('UPDATE Employee SET name = ?, email = ?, phone = ?, dob = ?, joining_date = ?, department = ?, manager_id = ?, rating_overall = ? WHERE employee_id = ?', [
            emp.name,
            emp.email,
            emp.phone || null,
            emp.dob || null,
...
app.post('/api/projects', async (req, res, next) => {
    try {
        const proj = req.body;
        await pool.execute('INSERT INTO ListOfProjects (project_name, department_id, starting_date, project_manager, co_manager, deadline, status, no_of_people_working, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            proj.project_name,
...
app.put('/api/projects/:id', async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        const proj = req.body;
        await pool.execute('UPDATE ListOfProjects SET project_name = ?, department_id = ?, starting_date = ?, project_manager = ?, co_manager = ?, deadline = ?, status = ?, no_of_people_working = ?, remarks = ? WHERE project_id = ?', [
            proj.project_name,
            proj.department_id || null,
            proj.starting_date,
            proj.project_manager,
...
app.post('/api/users', async (req, res, next) => {
    try {
        ...
        await pool.execute('INSERT INTO user (name, email, phone_no, dob, joining_date, department, password) VALUES (?, ?, ?, ?, ?, ?, ?)', [
            user.name,
            user.email,
            user.phone_no || null,
            user.dob || null,
...
app.put('/api/users/:id', async (req, res, next) => {
    try {
        ...
        if (user.password) {
            await pool.execute('UPDATE user SET name = ?, email = ?, phone_no = ?, dob = ?, joining_date = ?, department = ?, password = ? WHERE user_id = ?', [
                user.name,
                user.email,
                user.phone_no || null,
                user.dob || null,
            ...
            await pool.execute('UPDATE user SET name = ?, email = ?, phone_no = ?, dob = ?, joining_date = ?, department = ? WHERE user_id = ?', [
                user.name,
                user.email,
                user.phone_no || null,
                user.dob || null,
