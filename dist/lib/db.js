...
export async function assignProjectToEmployee(employeeId, projectId) {
    return executeQuery('INSERT INTO employeeprojects (employee_id, list_project_id) VALUES (?, ?)', [employeeId, projectId]);
}
export async function removeProjectFromEmployee(employeeId, projectId) {
    return executeQuery('DELETE FROM employeeprojects WHERE employee_id = ? AND list_project_id = ?', [employeeId, projectId]);
}
